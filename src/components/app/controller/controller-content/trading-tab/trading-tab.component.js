...
export class TradingTab extends BaseComponent {
	...
	#handleExportConfigs() {
		...
		const fileName = `trading-configs_${date}.json`;
	...
	#applyImportedConfig(json) {
		...
		Object.entries(json).forEach(([configId, newConfig]) => {
			if (!validateConfig(newConfig, TradingTab.REQUIRED_KEYS)) return;
	...
	#saveConfigToStorage(configId, config) {
		...
		storageService.setItem(STORAGE_KEYS.TRADING_CONFIGS, storedConfigs);
	}
	...
	#getStoredConfigs() {
		return storageService.getItem(STORAGE_KEYS.TRADING_CONFIGS) || {};
	}
}
